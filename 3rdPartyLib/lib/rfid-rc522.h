 ...
 *
 */
extern void RFID_RC522_Init(SPI_t* hspi);
 ...
 */
extern TM_MFRC522_Status_t TM_MFRC522_Check(SPI_t* hspi, uint8_t* id, uint8_t* type);
 ...
 */
extern TM_MFRC522_Status_t TM_MFRC522_Compare(SPI_t* hspi, uint8_t* CardID, uint8_t* CompareID);

/**
 * Private functions
 */

extern void TM_MFRC522_WriteRegister(SPI_t* hspi, uint8_t addr, uint8_t val);
extern uint8_t TM_MFRC522_ReadRegister(SPI_t* hspi, uint8_t addr);
extern void TM_MFRC522_SetBitMask(SPI_t* hspi, uint8_t reg, uint8_t mask);
extern void TM_MFRC522_ClearBitMask(SPI_t* hspi, uint8_t reg, uint8_t mask);
extern void TM_MFRC522_AntennaOn(SPI_t* hspi );
extern void TM_MFRC522_AntennaOff(SPI_t* hspi );
extern void TM_MFRC522_Reset(SPI_t* hspi );
extern TM_MFRC522_Status_t TM_MFRC522_Request(SPI_t* hspi, uint8_t reqMode, uint8_t* TagType);
extern TM_MFRC522_Status_t TM_MFRC522_ToCard(SPI_t* hspi, uint8_t command, uint8_t* sendData, uint8_t sendLen, uint8_t* backData, uint16_t* backLen);
extern TM_MFRC522_Status_t TM_MFRC522_Anticoll(SPI_t* hspi, uint8_t* serNum);
extern TM_MFRC522_Status_t TM_MFRC522_CalculateCRC(SPI_t* hspi, uint8_t* pIndata, uint8_t len, uint8_t* pOutData);
extern TM_MFRC522_Status_t TM_MFRC522_SelectTag(SPI_t* hspi, uint8_t* serNum, uint8_t* sak);
extern TM_MFRC522_Status_t TM_MFRC522_Auth(SPI_t* hspi, uint8_t authMode, uint8_t BlockAddr, uint8_t* Sectorkey, uint8_t* serNum);
extern TM_MFRC522_Status_t TM_MFRC522_Read(SPI_t* hspi, uint8_t blockAddr, uint8_t* recvData);
extern TM_MFRC522_Status_t TM_MFRC522_Write(SPI_t* hspi, uint8_t blockAddr, uint8_t* writeData);
extern void TM_MFRC522_Halt(SPI_t* hspi );
extern int TM_MFRC522_CS_Write(SPI_t* hspi, gpio_value_t val);
